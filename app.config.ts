...
export default ({ config }: ConfigContext): ExpoConfig => {
  ...
  return {
    ...
    ios: {
      ...
      supportsTablet: true,
      bundleIdentifier: process.env.BILT_IOS_BUNDLE_ID ?? 'me.bilt.myelginhub',
    },
    android: {
      package: process.env.BILT_ANDROID_PACKAGE ?? 'me.bilt.myelginhub',
    },
    web: {
      bundler: 'metro',
    ...
    plugins: [
      'expo-router',
      'expo-font',
      ['expo-notifications', { color: '#21599E' }],
      ...nativePlugins,
    ],
    experiments: {
      typedRoutes: true,
